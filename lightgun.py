...
class Config():
    def __init__(self):
            ...
            with open(LED_FILE) as f:
                ...
                try:
                    for line in f:
                        ...
                        elif l[0].lower() == "screenheightmm":
                            self.wiimoteLength = WIIMOTE_LENGTH_MM / float(l[1])
        ...
        if self.screenHeightMM is not None:
            self.updateYCorrection(self.screenHeightMM,self.sightHeightMM)
        elif self.yCorrection is None:
            self.yCorrection = 0

    def updateYCorrection(self,screenHeightMM,sightHeightMM):
        self.yCorrection = (sightHeightMM + WIIMOTE_Y_OFFSET_FROM_CAMERA) / screenHeightMM    
...
def getIRQuad(ir):
    ...
    if NUM_POINTS == 2:
        ...
        if 0 in identified and 1 in identified:
            ...
            if lastQuad is None or lastQuad[2] is None or lastQuad[3] is None:
                lastQuad = [p0,p1,None,None]
            else:
                if lastQuad[0] is None or lastQuad[1] is None:
            ...
            if lastQuad is None or lastQuad[0] is None or lastQuad[1] is None:
                lastQuad = [None,None,p2,p3]
            else:
                if lastQuad[2] is None or lastQuad[3] is None:
...
def measure(flexible=False):
    ...
    ledPixel = [[int(SCREEN_SIZE[0]*1./3),int(-0.1*SCREEN_SIZE[1])],[int(SCREEN_SIZE[0]*2./3),int(-0.1*SCREEN_SIZE[1])],[int(SCREEN_SIZE[0]*2./3),int(1.1*SCREEN_SIZE[1])],[int(SCREEN_SIZE[0]*1./3),int(1.1*SCREEN_SIZE[1])]]

    if CONFIG.ledLocations:
        for i in range(4):
            for j in range(2):
                if not math.isnan(CONFIG.ledLocations[i][j]):
                    ledPixel[i][j] = int(math.floor(0.5+CONFIG.ledLocations[i][j]*SCREEN_SIZE[j]))
    ...
    while running:
        ...
        if irQuad:
            CONFIG.setLEDLocations(ledPixel,SCREEN_SIZE)
        ...
        for wii,dir in buttonMap:
            if pressed & wii:
                nextRepeat = t+REPEAT_DELAY
                accelTime = t+ACCEL_DELAY
                move = dir
                break
        ...
        if corner<4:
            ...
            for i in range(NUM_POINTS):
                ...
                if bottom:
                    ...
                    length = y-SCREEN_SIZE[1]
                    signY = -1
                drawArrow(xy,bottom,color=YELLOW if i==corner else LIGHTER_GRAY)
                if i == corner:
            ...
            if NUM_POINTS == 2:
                if ledPixel[0][1] < .5*SCREEN_SIZE[1]:
            ...
            display.drawLEDOffsetArrow(None)
            ...
            display.drawLEDOffsetArrow(ledOffsetPix)
            display.delete("cameraCross")
        ...
        if pressed & wiimote.BTN_PLUS:
            ...
            if corner < 4:
                if NUM_POINTS == 4:
                    NUM_POINTS = 2
                elif ledPixel[0][1] < .5*SCREEN_SIZE[1]:
                    ledPixel[0][1] = SCREEN_SIZE[1]-ledPixel[0][1]
                    ledPixel[1][1] = SCREEN_SIZE[1]-ledPixel[1][1]
                else:
                    ledPixel[0][1] = SCREEN_SIZE[1]-ledPixel[0][1]
                    ledPixel[1][1] = SCREEN_SIZE[1]-ledPixel[1][1]
                    NUM_POINTS = 4
